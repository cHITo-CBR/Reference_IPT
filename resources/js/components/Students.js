...
export default function Students() {
    ...
    // Map status to badge class per SCSS
    const statusBadgeClass = (s) => {
        const k = String(s || '').toLowerCase();
        if (k === 'active') return 'badge badge-active';
        if (k === 'inactive') return 'badge badge-inactive';
        if (k === 'graduated') return 'badge badge-graduated';
        if (k === 'suspended') return 'badge badge-suspended';
        if (k === 'on leave' || k === 'on_leave' || k === 'on-leave') return 'badge badge-on-leave';
        if (k === 'archived') return 'badge badge-archived';
        return 'badge';
    };
